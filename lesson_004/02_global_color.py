...
while True:
    ...
    if color.isdigit():
        ...
        if color in colors:
            ...
            draw_triangle(start_point=point, angle=10, length=100, color=colors[color][1])

            point = sd.get_point(100, 400)
            draw_quadrate(start_point=point, angle=10, length=100, color=colors[color][1])

            point = sd.get_point(400, 100)
            draw_pentagon(start_point=point, angle=10, length=100, color=colors[color][1])

            point = sd.get_point(100, 100)
            draw_hexagon(start_point=point, angle=10, length=100, color=colors[color][1])
            break
        else:
